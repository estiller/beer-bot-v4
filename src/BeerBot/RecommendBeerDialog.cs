...
namespace BeerBot
{
    public class RecommendBeerDialog : ComponentDialog
    {
        ...
        public RecommendBeerDialog(string id, IBeerApi beerService) : base(id)
        {
            ...
            AddDialog(new WaterfallDialog(DialogIds.BeerConfirmation, new WaterfallStep[]
            {
                async (stepContext, cancellationToken) =>
                {
                    ...
                    switch (beers.Count)
                    {
                        ...
                        case 1:
                            await stepContext.Context.SendActivityAsync($"Eureka! This is the beer for you: '{beers[0].Name}'", cancellationToken: cancellationToken);
                            await stepContext.Context.SendActivityAsync($"Glad I could help {Emoji.Beer}", cancellationToken: cancellationToken);
