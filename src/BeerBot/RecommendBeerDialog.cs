        ...
        private void AddBeerConfirmationDialog()
        {
            ...
            AddDialog(new WaterfallDialog(DialogIds.BeerConfirmation, new WaterfallStep[]
            {
                ...
                async (stepContext, cancellationToken) =>
                {
                    ...
                    await stepContext.Context.SendActivityAsync($"Too bad {Emoji.Disappointed}", "Too bad", cancellationToken: cancellationToken);
                    return await stepContext.EndDialogAsync(cancellationToken: cancellationToken);
                }
            }));
            ...
            async Task SendBeerCardAsync(string beerName, ITurnContext turnContext, CancellationToken cancellationToken)
            {
                ...
                var activity = MessageFactory.Attachment(
                    new HeroCard(
                        ...
                        .ToAttachment());
                activity.Speak = $"This is your {beerName}";
                await turnContext.SendActivityAsync(activity, cancellationToken);
                await turnContext.SendActivityAsync($"Glad I could help {Emoji.Beer}", "Glad I could help", cancellationToken: cancellationToken);
            }
        }
            ...
            private async Task<PromptRecognizerResult<IList<Beer>>> RecognizeBeers(ITurnContext turnContext, CancellationToken cancellationToken, string beerName)
            {
                ...
                else
                {
                    ...
                    if (beers.Count == 0)
                    {
                        result.Succeeded = false;
                        await turnContext.SendActivityAsync(
                            "Oops! I haven't found any beer! Please try again.",
                            "Oops! I haven't found any beer! Please try again.",
                            cancellationToken: cancellationToken);
                    }
                    else
